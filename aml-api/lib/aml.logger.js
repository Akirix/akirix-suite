...
exports.log = function( logType, message, options ){
    ...
    if( _.indexOf( _this.logs, logType ) !== -1 ){
        ...
        if( _.isObject( log.req ) ){
            ...
            if( _.isObject( log.req.client ) ){
                if( _.isString( log.req.client.admin_id ) ){
                    logMessage += 'admin[' + log.req.client.admin_id + '] ';
                }

                if( _.isString( log.req.client.name ) && _.isString( log.req.client.id ) && _.isString( log.req.client.company_id ) ){
                    logMessage += log.req.client.name + '[' + log.req.client.id + '][' + log.req.client.company_id + '] ';
                }
            }
        }
        else{
