...
export default Ember.Controller.extend( EmberValidations.Mixin, {
    ...
    usingIntBank: function(){
        if( this.get( 'wireTemplate.intermediary_bank_name' ) ){
    ...
    }.observes( 'wireTemplate.intermediary_bank_name' ),
    ...
    actions: {
        updateTemplate: function(){
            ...
            this.validate().then(
                function(){
                    ...
                    self.get( 'wireTemplate' ).save().then(
                        function(){
                            self.set( 'isLocked', false );
                            self.notify.success( 'Wire Template updated.', { closeAfter: 10000 } );
        ...
        delete: function( template_id ){
            var self = this;
            self.get( 'wireTemplate' ).deleteRecord();
            self.get( 'wireTemplate' ).save().then(
                function(){
                    self.notify.success( 'The template has been removed.', { closeAfter: 5000 } );
                    self.transitionToRoute( 'companies.view.wire-templates' );
                },
                function( xhr ){
                    self.get( 'akxUtil' ).handleError( xhr, {
