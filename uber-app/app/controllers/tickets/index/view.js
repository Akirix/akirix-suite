...
export default Ember.Controller.extend( EmberValidations.Mixin, {
    ...
    needs: [ 'application', 'tickets/index', 'tickets' ],
    ...
    //    var input = this.get( 'newTag' ) || '';
    //    if( input.length > 2 ){
    //        tags.filter( function( tag ){
    //            if( tag.get( 'name' ).substring( 0, 3 ) === input.substring( 0, 3 ) ){
    //                self.set( 'newTag', tag.get( 'name' ) );
    //            }
    //        } );
    //    }
    //}.observes( 'newTag' ),
    ...
    loadMessages: function(){
        var self = this;
        this.store.find( 'ticket-message', { ticket_id: self.get( 'ticket.id' ) } )
        ...
        closeTicket: function(){
            ...
            Ember.$.ajax( {
                ...
                function(){
                    ...
                    self.transitionToRoute( 'tickets.index' );
                },
            ...
            Ember.run( function(){
                Ember.$.ajax( {
                    ...
                    function(){
                        self.set( 'isLocked', false );
                        self.send( 'refreshTicket' );
                        self.notify.success( 'Ticket added to pending.', { closeAfter: 5000 } );
                        self.transitionToRoute( 'tickets.index' );
                    },
                    function( xhr ){
                        self.get( 'akxUtil' ).handleError( xhr, {
