...
export default Ember.Route.extend( AuthenticatedRouteMixin, {
    ...
    setupController: function( controller, model ){
        ...
        if( model.get( 'type' ) === 0 ){
            ...
            self.store.find( 'account', { currency_id: model.get( 'currency_id' ), type: 0 } ).then( function( accounts ){
                controller.set( 'accounts', accounts );
            } );

            self.store.find( 'transaction', { model: 'invoice', model_id: model.id } ).then( function( transactions ){
                controller.set( 'invoice_transactions', transactions );
            } );
    ...
    renderTemplate: function( controller, model ){
        this.render( 'companies/invoices/view-bill', {
            into: 'companies',
    ...
    actions: {
        goBack: function(){
            ...
            this.transitionTo( 'companies.view.invoices.bills')
        }
    }
