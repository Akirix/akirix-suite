...
export default Ember.Component.extend( {
    ...
    renderComponent: function(){
        ...
        Ember.run( function(){
            Ember.$.ajax( {
                url: config.APP.uber_api_host + '/analytics/transactions-fx-volume?base_currency_id=' + self.get( 'currency.id' ) + '&from_date=' + self.get( 'from_date' ) + '&to_date=' + self.get( 'to_date' ),
                type: 'GET'
            } ).then(
                function( response ){
                    if( !Ember.isEmpty( response.data ) && !Ember.isEmpty( response.data[ 0 ].data ) ){
                        var seriesData = [];
