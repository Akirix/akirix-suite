...
exports.create = function( req, res, next ){
    ...
    if( _.isEmpty( req.validationErrors ) ){
        ...
        Invoice.find( req.params.invoiceItem.invoice_id ).done( function( err, invoice ){
            if( !!err ){
                _this.handleError( err, req, res );
                return next();
            }
                ...
                if( req.body.invoiceItem.quantity !== undefined && req.body.invoiceItem.quantity !== null && req.body.invoiceItem.quantity >= 0 ){
        ...
        InvoiceItem.find( req.params.invoice_item_id )
            .done( function( err, item ){
                if( !!err ){
                    ...
                    Invoice.find( item.invoice_id ).done( function( err, invoice ){
                        if( !!err ){
                            _this.handleError( err, req, res );
                            return next();
                        }
